...
## 2026-04-29 (team and manager workflows)

- `CM-BLG-115` Team and manager workflows: `MeetingPlaybook` struct (Codable, Identifiable, Equatable, Sendable) holds `name`, `meetingType`, `focusAreas`, `coachingCues`, `riskSignals`, `teamContext`, and `isBuiltIn`. Five built-in defaults ship for sales, demo, client-review, interview, and internal-sync meeting types. `PlaybookStore` manages disk persistence (custom playbooks only; built-ins are always prepended on load), plus `importPlaybook(from:)` (new UUID, marks custom) and `exportPlaybook(_:to:)`. `AppState` gains `activePlaybookID` (backward-compat, default ""). `ConversationRequest` gains `teamContext: String`; OpenAI and Ollama live-guidance prompts inject "Team context (always apply)" when non-empty. `BriefCoordinator.Input`, `MeetingBriefBuilder.BriefInput`, and both brief service request types carry `teamContext` through the brief pipeline. `AppModel` exposes `@Published var playbooks`, `activePlaybookID`, `activePlaybook`, plus `activatePlaybook(_:)`, `createPlaybook(name:meetingType:)`, `deletePlaybook(_:)`, `importPlaybook(from:)`, `exportPlaybook(_:)`, and `savePlaybooks()`. Settings gains a "Playbooks" card: active playbook selector, per-row play/export/delete actions, add-new form, and import-from-file button. Start screen gains an `activePlaybookBanner` when a playbook is active. 9 new tests covering built-in defaults, store round-trip, built-ins-not-on-disk invariant, import/export, and teamContext field; 89 total passing.

## 2026-04-29 (CRM and calendar integrations)

- `CM-BLG-114` CRM and calendar integrations: `CalendarEventParser` reads RFC 5545 ICS files — handles line unfolding, SUMMARY, DTSTART/DTEND (UTC, local, and all-day formats), ATTENDEE CN= extraction (quoted and unquoted), DESCRIPTION, and LOCATION. `CalendarEventRecord` holds parsed data and exposes `calendarContextSummary` for prompt injection. `AppModel` gains `importedCalendarEvent` (@Published, session-scoped), `importCalendarEvent(from:)`, and `clearCalendarEvent()`. `ConversationRequest` gains `calendarContext: String`; `BriefCoordinator.Input`, `MeetingBriefBuilder.BriefInput`, `OllamaBriefGenerationRequest`, and `OpenAIBriefGenerationRequest` all carry calendar context through the brief pipeline. The heuristic builder appends calendar data to the `priorSessionNote` field. OpenAI and Ollama live-guidance prompts inject a "Calendar event context" section when non-empty. Meeting Context card gains "Import Calendar Event (.ics)" button with a file picker, imported event display (title, date, attendees, agenda preview), and a Clear button. 13 new tests covering ICS parsing (title, attendees, dates, description, errors, escaping, context summary) and ConversationRequest field; 80 total passing.
